...
> Status: Draft
> Constitution check: Principles I, III, V, VIII
...
**Given** I submit a review (1-5 stars + optional text),
...
| FR-02 | Teacher application with credentials and optional proof document upload | P0 |
...
| FR-06 | Event-teacher assignment: event creators pick from verified teachers when creating events | P0 |
| FR-07 | Post-event reviews: 1-5 stars + optional text; only by confirmed attendees | P2 |
...
- **TeacherProfile**: id, userId, bio, specialties (array), createdAt, updatedAt
...
- **Review**: id, eventId, teacherProfileId, reviewerId, rating (1-5), text, createdAt
...
- Review moderation: initially no moderation; if abuse is reported, admin can hide a review (soft-delete with reason)
- Teacher leaves platform: profile becomes "inactive"; reviews and history remain for community benefit; PII removed per GDPR

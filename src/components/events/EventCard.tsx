  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mt-3 flex-wrap">
        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${categoryColors[event.category] ?? "bg-gray-100"}`}>
          {event.category.replace("_", " ")}
        </span>
        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${skillColors[event.skillLevel] ?? "bg-gray-100"}`}>
          {event.skillLevel.replace("_", " ")}
        </span>

        {event.isNew && (
          <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800">New</span>
        )}
        {event.isUpdated && (
          <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-sky-100 text-sky-800">Updated</span>
        )}
        {isFull && (
          <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-red-100 text-red-800">Full</span>
        )}
      </div>
